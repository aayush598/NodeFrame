...
export const ApiCallNode: React.FC<NodeProps<CustomNodeData>> = ({ id, data, selected, isConnectable }) => {
  ...
  return (
    <div
      ...
      style={{ borderColor: status === 'idle' ? (data.color || '#06b6d4') : undefined }}
    >
      {/* Status Badge */}
      ...
      <div className="flex items-center gap-2">
        ...
        >
          {data.icon || <Globe className="w-4 h-4 text-white" />}
        </div>
        <div className="flex-1">
          <div className="font-semibold text-sm text-gray-900">{data.label || 'API Call'}</div>
          {data.description && (
